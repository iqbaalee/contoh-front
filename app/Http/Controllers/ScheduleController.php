...
class ScheduleController extends Controller
{
        ...
        try {
            ...
            if (!empty($request->day) || !empty($request->month) || !empty($request->year)) {
                ...
                if (!empty($request->day) && !empty($request->month) && !empty($request->year)) {
                    ...
                    return redirect()->route('schedule.index')->with('error', 'Bulan jangan dikosongkan');
                ...
                $listHour = json_decode($this->apiService->restApi('get', 'product', [
                    'filter' => $filter
                ]));
            } else {
                $listHour = json_decode($this->apiService->restApi('get', 'product'));
    ...
    public function index()
    {
        ...
        return view('pages.schedule.index', ['title' => 'Jadwal', 'dateNow' => $dateNow, 'listYear' => $listYear, 'listMonth' => $listMonth]);
    }
    ...
    public function detail($id, Request $request)
    {
        try {
            if (!empty($request->order_id)) {
                ...
                $response = $this->apiService->restApi('get', 'product/' . $id, ['order_id' => $request->order_id]);
            } else {
                $response = $this->apiService->restApi('get', 'product/' . $id);
            }
            return $response;
        } catch (\Exception $e) {
            return ['code' => 400, 'status' => 'error', 'message' => $e->getMessage()];
